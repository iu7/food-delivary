...
def restaurant_menu_item_create(restaurant_id, menu_id):
	...
	try:
		mitem = MenuItem(menu_id, title, price, info, bonus)
...
def restaurant_order_confirmation(restaurant_id):
	...
	online_payment = request.json.get('online_payment')
	delivery_payment = request.json.get('delivery_payment')
	bonus_price = request.json.get('bonus_price')
	if online_payment: online_payment = True
	if not title_list or not order_list or not price_list or not cdata or not total:
		raise UException('Incorrect request')
	try:
		if bonus_price:	bonus_price = float(bonus_price)
		else: bonus_price = 0
		restaurant = Restaurant.query.filter_by(id=restaurant_id).first()
		order = Orders(restaurant_id, restaurant.name, online_payment, total, delivery_payment, bonus_price, user_id)
		db.session.add(order)
		db.session.flush()
		...
		db.session.commit()
		points = order.points - order.bonus_price
	except Exception as exc:
		db.session.rollback()
		raise UException(message='Unexpected server exception', status_code=500, payload=exc.message)
...
def restaurant_history():
	...
	if not restaurant_id:
		raise UException('Incorrect restaurant_id')
	return jsonify(orders=restaurant.get_order_list(confirmed, canceled))

@main.route('/restaurant/<int:restaurant_id>/order/<int:order_id>/status/change', methods=['PUT'])
def restaurant_order_status_change(restaurant_id, order_id):
	status_type = request.args.get('status_type')
	if not status_type or status_type != 'canceled' and status_type != 'confirmed':
		raise UException('Incorrect request')
	order = Orders.query.filter_by(id=order_id).first()
	if not order or order.restaurant_id != restaurant_id:
		raise UException('Incorrect request')
	...
	try:
		...
		if status_type == 'confirmed':
			order.confirmed = not order.confirmed
			value = order.confirmed
		elif status_type == 'canceled':
			order.canceled = not order.canceled
			value = order.canceled
		db.session.commit()
		points = 0
		print order
		if order.user_id:
			if order.confirmed and not order.canceled:
				points = order.points - order.bonus_price
			elif order.canceled and not order.confirmed:
				points = 0
			else:
				points = order.bonus_price - order.points
		user_id = order.user_id
	except Exception as exc:
		raise UException(message='Unexpected server exception', status_code=500, payload=exc.message)
	return jsonify(status_type=status_type, value=value, points=points, user_id=user_id)
