...
class Orders(db.Model):
	...
	def __repr__(self):
		return '<Orders: id: %s, user_id: %s, restaurant_id: %s, date: %s, confirmed: %s' \
			% (str(self.id), str(self.user_id), str(self.restaurant_id),\
				self.date.strftime(RestaurantConfig.DTIME_FORMAT), str(self.confirmed))

	def get_order(self):
		olist = []
		for order in self.order_list:
			olist.append(order.get_order_list())
		return {'order' : {'id' : self.id, 'user_id' : self.user_id, 'confirmed' : self.confirmed, \
			'restaurant_id' : self.restaurant_id, \
			'date' : self.date.strftime(RestaurantConfig.DTIME_FORMAT), \
			'order_list' : olist} }
	...
	def validate_user_id(self, key, value):
		if type(value) is not type(int()):
			raise ValueError('Incorrect user_id format')
		return value
