...
class Restaurant(db.Model):
	...
	@staticmethod
	def get_restaurant_list(activated):
		if activated is None:
			restaurants = Restaurant.query.all()
		else:
			restaurants = Restaurant.query.filter_by(activated=activated).all()
		result = []
		for restaurant in restaurants:
			attr = restaurant.attributes[0]
			officials = restaurant.officials[0]
			result.append({'restaurant_id':restaurant.id, 'name':restaurant.name, 'user_id':restaurant.user_id,\
					'telephone':restaurant.telephone, 'order_email':restaurant.email,\
					'attributes' : attr.get_attributes(), 'cuisines':restaurant.get_cuisines(),\
					'officials':officials.get_officials(), 'activated':restaurant.activated})
		return result
...
class MenuItem(db.Model):
	...
	def __init__(self, menu_id, title, price, info, bonus):
		self.title = title
		self.price = price
	...
	def get_menu_item(self):
		return {'menu_item_id' : self.id, 'title' : self.title, 'price' : self.price, 'info':self.info, 'bonus' : self.bonus}
...
class Orders(db.Model):
	...
	bonus_price = db.Column(db.Float, default=0)
	...
	def __init__(self, restaurant_id, restaurant_name, online_payment, total, delivery_payment, bonus_price, user_id):
		self.restaurant_id = restaurant_id
		self.user_id = user_id
		...
		self.canceled = False
		self.info = ''
		if bonus_price is None: self.bonus_price = 0.0
		else: self.bonus_price = bonus_price
		self.delivery_payment = delivery_payment
		self.total = float(total) + float(delivery_payment)
		self.points = round((float(total) + float(delivery_payment)) / RestaurantConfig.BONUS_PERCENTS, 2)
