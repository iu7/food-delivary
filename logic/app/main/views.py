...
def user_list(role):
	...
	if flag:
		...
		if role == 'Client':
			...
			flag, result = furls.restaurant_list(activated)
			for restaurant in result['restaurant_list']:
				for user in users['users']:
					if user['user_id'] == restaurant['user_id']:
...
def restaurant_activated(restaurant_id):
	if not restaurant_id:
		raise UException('Incorrect request')
	flag, result = furls.restaurant_activated_change(restaurant_id)
	if flag: code = 200
	else: code = result['status_code']
	return jsonify(result), code
...
def create_cuisines():
	...
	if not title:
		raise UException('Incorrect title')
	flag, result = furls.restaurant_cuisine_create(title)
	if flag: code = 201
	else: code = result['status_code']
	return jsonify(result), code
...
def restaurant_additional_attr_delete():
	...
	if not data:
		raise UException('Incorrect request')
	flag, result = furls.restaurant_attributes_add_delete(data)
	if flag: code = 200
	else: code = result['status_code']
	return jsonify(result), code
